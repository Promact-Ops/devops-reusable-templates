# Main Terraform configuration for AWS ECS with EC2 infrastructure
# This configuration creates VPC, S3, ECS (EC2 only with ASG), ALB, and RDS resources
...
module "vpc" {
  ...
  # Enable NAT Gateway for private subnets (required for EC2 instances to pull images)
...
resource "aws_security_group" "rds_sg" {
  ...
  # Allow PostgreSQL from ECS instances
  ingress {
...
resource "aws_launch_template" "ecs_launch_template" {
  ...
  tag_specifications {
    ...
    tags = merge(local.common_tags, {
      Name = "${var.project_name}-${var.environment}-ecs-volume"
    })
  }
...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu_reservation" {
  alarm_name          = "${var.project_name}-${var.environment}-ecs-cpu-reservation"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "2"
...
resource "aws_cloudwatch_metric_alarm" "ecs_memory_reservation" {
  alarm_name          = "${var.project_name}-${var.environment}-ecs-memory-reservation"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "2"
